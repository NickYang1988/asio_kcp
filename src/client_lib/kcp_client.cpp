...
void kcp_client::init_kcp(kcp_conv_t conv)
{
    ...
    ikcp_nodelay(p_kcp_, 1, 5, 1, 1); // 3rd param 5 means interval time is 5 ms
    ikcp_wndsize(p_kcp_, 256, 256);
    ikcp_setmtu(p_kcp_, 470);
}
...
void kcp_client::do_send_connect_packet(uint64_t cur_clock)
{
    ...
    if (send_ret < 0)
    {
        std::cerr << "do_asio_kcp_connect send error return with errno: " << errno << " " << strerror(errno)
                  << std::endl;
    }
}
...
void kcp_client::try_recv_connect_back_packet(void)
{
    ...
    if (ret_recv < 0)
    {
        ...
        if (err == EAGAIN)
            return;
        std::cerr << "try_recv_connect_back_packet recv error return with errno: " << err << " " << strerror(err)
                  << std::endl;
    }
...
int kcp_client::init_udp_connect(void)
{
    ...
    // set socket non-blocking
    {
        ...
        if (flags == -1)
        {
            std::cerr << "get socket non-blocking: fcntl error return with errno: " << errno << " " << strerror(errno)
                      << std::endl;
        ...
        if (ret == -1)
        {
            std::cerr << "set socket non-blocking: fcntl error return with errno: " << errno << " " << strerror(errno)
                      << std::endl;
            return KCP_ERR_SET_NON_BLOCK_FAIL;
        }
