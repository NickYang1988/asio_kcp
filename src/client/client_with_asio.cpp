...
void client_with_asio::print_recv_log(const std::string& msg)
{
    ...
    if (cur_time - static_last_refresh_time > 10 * 1000 && static_good_recv_count % 100 == 30 &&
        static_good_recv_count != 30)
    {
        std::cout << " " << static_cast<double>(static_recved_bytes * 10 / (cur_time - static_last_refresh_time)) / 10
                  << "KB/s(in)";
